...
public abstract class TaskWeb<T extends ftc.evlib.tasks.Task, B extends StateMachineBuilder> {
...
//        builder = createBuilder(startState);
    ...
    public void addStages(List<ftc.evlib.tasks.Stage<T>> stages) {
        for (int i = 0; i < stages.size(); i++) {
            ftc.evlib.tasks.Stage<T> stage = stages.get(i);
            ...
            if (i < stages.size() - 1) {
                ftc.evlib.tasks.Stage<T> nextStage = stages.get(i + 1);
    ...
    public static void notImplemented(ftc.evlib.tasks.Task from, ftc.evlib.tasks.Task to) {
        notImplemented("Path between " + from + " and " + to);
    }

    public static void notImplemented(ftc.evlib.tasks.Task task) {
...
//        throw new UnsupportedOperationException(message);
    ...
    public static void notPossible(ftc.evlib.tasks.Task from, ftc.evlib.tasks.Task to) {
