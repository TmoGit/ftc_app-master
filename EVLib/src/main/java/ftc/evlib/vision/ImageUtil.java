...
public class ImageUtil {
    ...
    public static final List<Scalar> HSV_THRESHOLD_MIN;
    public static final List<Scalar> HSV_THRESHOLD_MAX;
    ...
    static {
        ...
        HSV_THRESHOLD_MIN = new ArrayList<>();
        HSV_THRESHOLD_MAX = new ArrayList<>();

        int h = ImageUtil.COLOR_WHEEL_START;
        for (int i = 0; i < 3; i++) {
            ImageUtil.HSV_THRESHOLD_MIN.add(new Scalar((h + ImageUtil.COLOR_WHEEL_GAP) / 2, ImageUtil.COLOR_WHEEL_S_MIN, ImageUtil.COLOR_WHEEL_V_MIN));
            h += 120;
            if (h > 360) h -= 360;
            ImageUtil.HSV_THRESHOLD_MAX.add(new Scalar((h - ImageUtil.COLOR_WHEEL_GAP) / 2, ImageUtil.COLOR_WHEEL_S_MAX, ImageUtil.COLOR_WHEEL_V_MAX));
        }
    }
     ...
     */
    public static boolean log2DArray(File dir, String beforeTimestamp, long time, String afterTimestamp, List<String> titles, int[][] values) {
        File file = new File(dir, beforeTimestamp + time + afterTimestamp);
