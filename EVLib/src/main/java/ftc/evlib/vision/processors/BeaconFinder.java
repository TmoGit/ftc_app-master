...
import ftc.evlib.vision.processors.CircleMatch;
...
//  private static final double MIN_MASS = 6; //minimum mass for column sum
    ...
    public ImageProcessorResult<BeaconPositionResult> process(long startTime, Mat rgbaFrame, boolean saveImages) {
        ...
        List<ftc.evlib.vision.processors.Circle> circles = new ArrayList<>();
...
//        Log.i(TAG, "Ellipse Area: " + area);
            ...
            if (ellipse != null) {
                if (isValid) {
                    ...
                    circles.add(new ftc.evlib.vision.processors.Circle(ellipse));
        ...
        if (circles.size() >= 2) {
            ...
            List<ftc.evlib.vision.processors.CircleMatch> matches = new ArrayList<>();
            ...
            for (int i = 0; i < circles.size() - 1; i++) {
                ...
                for (int j = i + 1; j < circles.size(); j++) {
                    matches.add(new ftc.evlib.vision.processors.CircleMatch(circles.get(i), circles.get(j), hsv.size()));
                }
            }
